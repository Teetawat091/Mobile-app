...
local function loadBird()
    ...
    bird = display.newImage(b .. ".png", x, y)
    if (b >= 8) then
        bird.id = "pokemon"
    else
        bird.id = "angry"
...
display.newImage("background.png", cx, cy)
...
sndHit = audio.loadSound("hit.mp3")
sndFailed = audio.loadSound("failed.mp3")
sndBackground = audio.loadSound("background.mp3")
